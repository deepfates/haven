defmodule HavenWeb.RunLiveTest do
  ...
  test "renders a conversation-first run thread with details disclosed on demand", %{conn: conn} do
    ...
    assert has_element?(view, ~s|#run-prompt[placeholder="Message the agent"]|)
    assert has_element?(view, "#run-prompt.min-h-20")
    assert has_element?(view, "#run-prompt.resize-y")
    assert has_element?(view, "#send-prompt-button", "Send")
    assert has_element?(view, "#cancel-run-button", "Cancel")
  ...
  test "surfaces other runs needing decisions while reading one run", %{conn: conn} do
    current = insert_disconnected_run!("Current conversation")
